        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL_SIGNUP, new Response.Listener<String>() {
            ...
            public void onErrorResponse(VolleyError error) {
                if (error instanceof TimeoutError) {
                    ...
                    global.openDialogError(SignupActivity.this,"Server terlalu lama merespon! coba lagi nanti");
                } else if (error instanceof NoConnectionError) {
                    //This indicates that the reuest has no connection
                    global.openDialogError(SignupActivity.this,"Check koneksi internet Anda!");
                } else if (error instanceof AuthFailureError) {
                    //Error indicating that there was an Authentication Failure while performing the request
                    global.openDialogError(SignupActivity.this,"Authentication gagal! coba beberapa saat lagi");
                } else if (error instanceof ServerError) {
                    //Indicates that the server responded with a error response
                    global.openDialogError(SignupActivity.this,"Server Error!");
                } else if (error instanceof NetworkError) {
                    //Indicates that there was network error while performing the request
                    global.openDialogError(SignupActivity.this,"Network Anda mengalami error!");
                } else if (error instanceof ParseError) {
                    // Indicates that the server response could not be parsed
                    global.openDialogError(SignupActivity.this,"Server tidak bisa parse permintaan Anda!");
                }
            }
        }) {
            @Override
